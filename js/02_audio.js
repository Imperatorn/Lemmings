...
const UNDERWATER_PANIC_GAIN_BOOST=1.34;
...
const AU={
  ...
  musicGainTarget(kind){
    ...
    const mysteryBoost=kind==='caveMystery'?CAVE_MYSTERY_GAIN_BOOST:(kind==='underwaterMystery'?UNDERWATER_MYSTERY_GAIN_BOOST:(kind==='underwaterPanic'?UNDERWATER_PANIC_GAIN_BOOST:1));
  ...
  sWaterfallCaveTeleportCharge(){
    ...
    this.padTone(392,1.78,'sine',0.010,t+0.18);
    this.tone(880,0.13,'sine',0.020,1.03,t+0.02);
    this.tone(1320,0.12,'triangle',0.014,0.98,t+0.08);
    this.softNoise(1.82,0.010,980,1.66,t+0.08,{type:'bandpass',q:0.45,smooth:0.88,attack:0.20,release:0.48});
    this.softNoise(1.36,0.006,4200,1.20,t+0.26,{type:'highpass',smooth:0.62,attack:0.18,release:0.36});
    const notes=[440,554,659,831,988];
    for(let i=0;i<notes.length;i++){
      const when=t+0.16+i*0.22;
    ...
    this.padTone(523,0.68,'sine',0.018,t+1.36);
    this.padTone(784,0.62,'triangle',0.016,t+1.42);
    this.tone(1568,0.22,'sine',0.030,1.02,t+1.64);
    this.tone(2093,0.16,'triangle',0.020,0.94,t+1.80);
    this.softNoise(0.34,0.012,5600,0.62,t+1.60,{type:'bandpass',q:0.72,smooth:0.72,attack:0.06,release:0.20});
  },
  ...
  sWaterfallCavePedestalRise(){
    if(!this.rateFx('waterfall-cave-pedestal-rise',2.90))return;
    const t=this.now();
    const bassDest=this.boomGain||this.sfxDest();
    this.tone(34,1.85,'sine',0.040,1.55,t+0.01,bassDest);
    this.tone(46,2.35,'triangle',0.028,0.78,t+0.24,bassDest);
    this.softNoise(2.48,0.024,95,0.52,t+0.02,{type:'lowpass',smooth:0.97,attack:0.10,release:1.05,dest:bassDest});
    this.tone(54,0.58,'sine',0.030,0.68,t);
    this.tone(68,0.62,'sine',0.034,0.74,t+0.01);
    this.tone(104,0.30,'triangle',0.020,0.78,t+0.10);
    this.softNoise(2.58,0.022,210,0.52,t,{type:'lowpass',smooth:0.95,attack:0.07,release:0.92});
    this.softNoise(2.04,0.018,620,0.76,t+0.08,{type:'bandpass',q:0.42,smooth:0.90,attack:0.05,release:0.66});
    this.softNoise(1.68,0.011,1760,0.70,t+0.25,{type:'bandpass',q:0.62,smooth:0.74,attack:0.09,release:0.54});
    const scrapes=[
      [0.20,240,0.019],[0.35,285,0.017],[0.52,210,0.018],
      [0.70,325,0.016],[0.90,250,0.014],[1.12,380,0.013],
      [1.34,300,0.012],[1.55,430,0.010],[1.82,340,0.009]
    ];
    ...
    const groans=[[0.06,92,0.012,0.56],[0.48,118,0.010,0.64],[0.94,86,0.011,0.60],[1.42,136,0.009,0.72],[1.94,104,0.008,0.68]];
    for(const g of groans)this.tone(g[1],0.50,'sine',g[2],g[3],t+g[0]);
    const subLifts=[[0.42,39,0.022,1.42],[1.10,43,0.020,1.30],[1.78,50,0.018,0.74],[2.34,64,0.017,0.62]];
    for(const b of subLifts)this.tone(b[1],0.52,'sine',b[2],b[3],t+b[0],bassDest);
    const surges=[[0.16,0.015,560],[0.46,0.014,880],[0.78,0.012,1180],[1.12,0.011,1540],[1.48,0.010,1960],[1.92,0.009,2300]];
    for(const w of surges)this.softNoise(0.24,w[1],w[2],0.66,t+w[0],{type:'bandpass',q:0.55,smooth:0.74,attack:0.018,release:0.16});
    const droplets=[[1.56,1760,0.008],[1.70,2349,0.007],[1.86,3136,0.006],[2.04,2637,0.005],[2.30,1976,0.005]];
    for(const d of droplets)this.tone(d[1],0.075,'sine',d[2],0.90,t+d[0]);
    this.padTone(196,1.72,'sine',0.010,t+0.24);
    this.padTone(294,1.48,'triangle',0.010,t+0.50);
    this.padTone(392,1.06,'sine',0.012,t+1.03);
    this.padTone(587,0.92,'triangle',0.011,t+1.22);
    ...
    this.tone(72,0.36,'sine',0.026,0.54,t+2.42,bassDest);
    this.softNoise(0.38,0.011,3200,0.58,t+1.60,{type:'bandpass',q:0.84,smooth:0.62,attack:0.05,release:0.22});
    this.softNoise(0.42,0.007,5600,0.50,t+1.82,{type:'bandpass',q:0.72,smooth:0.68,attack:0.04,release:0.22});
    this.softNoise(0.50,0.018,78,0.44,t+2.34,{type:'lowpass',smooth:0.96,attack:0.04,release:0.36,dest:bassDest});
  },
  sPortalStoneOpen(){
  ...
  PAT:{
    ...
    underwaterPanic:{bpm:124,
      mel:[0,49,0,50, 0,46,0,49, 0,52,0,50, 0,46,0,43,
           0,49,0,55, 0,50,0,46, 0,52,0,58, 0,49,0,46,
           0,47,0,50, 0,44,0,47, 0,53,0,50, 0,44,0,41,
           0,47,0,56, 0,50,0,44, 0,53,0,59, 0,47,0,44],
      bass:[22,0,22,0, 21,0,22,0, 22,0,25,0, 21,0,22,0,
            19,0,19,0, 18,0,19,0, 19,0,22,0, 18,0,19,0,
            17,0,17,0, 16,0,17,0, 17,0,20,0, 16,0,17,0,
            15,0,15,0, 14,0,15,0, 15,0,18,0, 14,0,15,0],
      harm:[0,0,56,0, 53,0,0,0, 0,0,59,0, 53,0,0,0,
            0,0,55,0, 52,0,0,0, 0,0,58,0, 52,0,0,0,
            0,0,54,0, 51,0,0,0, 0,0,57,0, 51,0,0,0,
            0,0,53,0, 50,0,0,0, 0,0,56,0, 50,0,0,0]},
    city:{bpm:128,
  ...
  retroLeadLayer(kind,m,i,t,stepDur,leadLen){
    ...
    }else if(kind==='underwaterPanic'){
      if(i%4===1)this.tone(this.midi(m+7),stepDur*0.30,'square',0.0075,0.985,t+stepDur*0.02,this.musGain);
      if(i%8===5)this.tone(this.midi(m+12),stepDur*0.22,'triangle',0.006,0.99,t+stepDur*0.10,this.musGain);
  ...
  retroBassLayer(kind,b,i,t,stepDur){
    ...
    if(kind==='day'){
      ...
      third=3;pulseVol=0.040;arpVol=0.010;pulseEvery=2;arpEvery=16;padVol=0;subVol=0.104;subEvery=4;subLen=2.15;
  ...
  startUnderwaterCavePanicMusic(fade){
    this.init();
    if(!this.ctx||!this.on||!this.musicOn)return false;
    if(this.mus&&this.mus.timer&&this.mus.kind==='underwaterPanic'){this.applyVolumes();return true}
    this.startMusic('underwaterPanic');
    if(!this.musGain||!this.musGain.gain)return true;
    const t=this.now(), dur=fade==null?0.22:Math.max(0.03,fade);
    const target=MUSIC_GAIN_BASE*UNDERWATER_PANIC_GAIN_BOOST*clamp(Number.isFinite(this.musicVol)?this.musicVol:1,0,1);
    const p=this.musGain.gain;
    try{
      if(p.cancelScheduledValues)p.cancelScheduledValues(t);
      if(p.setValueAtTime)p.setValueAtTime(0.00005,t);
  ...
  stopUnderwaterCavePanicMusic(fade){
    if(!this.mus||this.mus.kind!=='underwaterPanic')return false;
    this.silenceMusic(fade==null?0.18:fade);
    return true;
  },
  pump(){
    if(!this.ctx)return;
    ...
    while(this.mus.next<this.now()+0.25){
      ...
      const underwaterPanic=kind==='underwaterPanic';
      const mystery=kind==='caveMystery'||underwaterMystery||underwaterPanic;
      const lead=underwaterPanic?'triangle':(kind==='cave'||mystery?'sine':(kind==='menu'?(mp>=64&&mp<96?'triangle':'square'):(kind==='lava'||kind==='night'||kind==='desert'?'triangle':(kind==='day2'&&i%32>=16?'triangle':'square'))));
      const accent=kind==='day2'?(i%16===0?1.18:(i%8===6?0.86:1.0)):(kind==='menu'?(mp>=96?1.10:(mp>=64?0.88:(mp%32===0?1.06:1.0))):1);
      const leadVol=underwaterPanic?0.052:(mystery?0.066:(kind==='cave'?0.075:(kind==='lava'?0.102:(kind==='night'?0.10:(kind==='desert'?0.066:(kind==='menu'?0.050*accent:(kind==='city'?0.068:(kind==='day2'?0.060*accent:0.07))))))));
      const leadLen=underwaterPanic?stepDur*0.46:(mystery?stepDur*3.6:(kind==='cave'?stepDur*2.55:(kind==='lava'?stepDur*2.25:(kind==='menu'?stepDur*(P.mel[(i+1)%P.mel.length]?0.62:(mp>=64&&mp<96?1.05:0.88)):(kind==='desert'?stepDur*(i%8===0?1.65:1.05):(kind==='city'?stepDur*(P.mel[(i+1)%P.mel.length]?0.84:1.28):(kind==='day2'?(i%16===14?stepDur*1.55:(i%4===0?stepDur*1.10:stepDur*0.82)):stepDur*0.95)))))));
      ...
      if(P.harm){
        ...
        if(h)this.tone(this.midi(h),underwaterPanic?stepDur*0.42:(mystery?stepDur*3.2:(kind==='cave'?stepDur*2.2:(kind==='lava'?stepDur*2.05:(kind==='menu'?stepDur*(mp>=64&&mp<96?1.12:0.78):(kind==='desert'?stepDur*1.75:stepDur*1.35))))),'triangle',underwaterPanic?0.022:(mystery?0.026:(kind==='cave'?0.024:(kind==='lava'?0.034:(kind==='menu'?0.014*accent:(kind==='desert'?0.022:0.030*accent))))),1,t+stepDur*0.08,this.musGain);
      }
      if(b)this.tone(this.midi(b),underwaterPanic?stepDur*0.92:(mystery?stepDur*4.4:(kind==='cave'?stepDur*2.8:(kind==='lava'?stepDur*3.05:(kind==='menu'?stepDur*(mp>=64&&mp<96?1.55:1.05):(kind==='day2'||kind==='city'?stepDur*1.35:stepDur*1.8))))),'triangle',underwaterPanic?0.242:(mystery?0.146:(kind==='cave'?0.158:(kind==='lava'?0.158:(kind==='night'?0.225:(kind==='desert'?0.130:(kind==='menu'?0.112:(kind==='day2'?0.140:0.158))))))),1,t,this.musGain);
      ...
      if(mystery){
        if(underwaterPanic){
          if(i%4===0)this.softNoise(0.34,0.020,180,0.78,t,{type:'lowpass',smooth:0.86,attack:0.010,release:0.13,dest:this.musGain});
          if(i%8===2&&b)this.tone(this.midi(b+12),stepDur*0.28,'square',0.026,0.97,t+stepDur*0.12,this.musGain);
          if(i%16===10)this.softNoise(0.18,0.014,900,0.62,t+stepDur*0.18,{type:'bandpass',q:0.75,smooth:0.55,attack:0.012,release:0.10,dest:this.musGain});
          if(i%32===30)this.tone(this.midi(55),stepDur*0.62,'sine',0.019,1.04,t+stepDur*0.05,this.musGain);
        }else{
        if(i%32===0){
          const root=b||28;
