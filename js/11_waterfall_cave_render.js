...
function waterfallCaveAdventureStyle(scene){
  const styles={
    emberPassage:{wall:'#140f0d',mid:'#211713',floor:'#172027',rim:'#5b3320',accent:'#ff9a3a',glow:'#d46b32'},
...
function drawWaterfallCaveAdventureBase(c,cave,tk,style){
  ...
  if(scene==='emberPassage'){
    c.globalAlpha=0.20;
    c.fillStyle='#ff8a3a';
    fillPixelPoly(c,[[CW,94],[CW,CH],[278,CH],[334,206],[392,132]]);
    c.globalAlpha=0.15;
    c.fillStyle='#ffd080';
...
function drawWaterfallCaveWallTorch(c,x,y,tk,obj){
  ...
  c.fillStyle='#ff9a3a';
  fillPixelPoly(c,[[x,y-18],[x-70,y+28],[x-42,y+106],[x+46,y+104],[x+76,y+30]]);
  c.globalAlpha=1;
...
function drawWaterfallCaveAdventureDetails(c,cave,tk,style){
  ...
  if(scene==='emberPassage'){
    ...
    for(let i=0;i<5;i++){
      const y=114+i*24;
      c.fillRect(80,y,74,4);
      c.fillRect(326,y+8,66,4);
      c.fillStyle='#453020';
      c.fillRect(88,y-8,18,8);
      c.fillRect(336,y,14,8);
      c.fillStyle='#21160f';
    }
    c.globalAlpha=0.32;
    c.fillStyle='#d5a55a';
    for(let i=0;i<16;i++){
      const x=104+Math.round(hash2(i+887,wf.x||0)*270);
      const y=92+Math.round(hash2(i+889,wf.y||0)*136);
      c.fillRect(x,y,8,1);
      c.fillRect(x+3,y-4,1,8);
...
function drawWaterfallCaveAdventureObjects(c,cave,tk,style){
  ...
  for(const hit of objects){
    ...
    if(kind==='torch'){
      ...
      c.fillStyle='#160f0b';c.fillRect(x-74,y-54,148,84);
      c.fillStyle='#352519';c.fillRect(x-68,y-48,136,72);
      c.fillStyle=active?'#f0c76a':'#75532d';
      for(let i=0;i<18;i++){
        const gx=x-54+(i%6)*22, gy=y-34+Math.floor(i/6)*22;
        c.fillRect(gx,gy,10,2);c.fillRect(gx+4,gy-5,2,12);
        if((i+Math.floor(tk/20))%3===0)c.fillRect(gx+10,gy+4,5,2);
      }
      ...
      c.globalAlpha=0.34;
      c.fillStyle='#000000';
      fillPixelPoly(c,[[-24,7],[-12,2],[20,3],[28,8],[14,12],[-20,12]]);
      if(glow){
        c.globalAlpha=0.12+0.18*pulse;
        c.fillStyle=style.glow||'#d5a55a';
        fillPixelPoly(c,[[-34,8],[-18,-14],[24,-13],[38,7],[18,18],[-20,18]]);
      }
      c.globalAlpha=1;
      c.fillStyle='#3a2619';
      c.fillRect(-19,-11,38,22);
      c.fillStyle=near?'#eee0bd':'#cdbf9c';
      c.fillRect(-17,-9,34,18);
      c.fillStyle='#3275a8';
      c.fillRect(-15,-7,30,7);
      c.fillStyle='#e7dfc8';
      c.fillRect(-15,0,30,7);
      c.fillStyle='#7a4a2a';
      c.fillRect(-13,2,8,3);
      c.fillRect(-1,1,4,4);
      c.fillRect(8,2,4,3);
