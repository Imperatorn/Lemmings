...
function drawHolyLemmingAura(c,l,sx,sy,sc){
  ...
  const headY=y-Math.round(12*s);
  const hw=Math.max(3,Math.round(6*s)),hh=Math.max(1,Math.round(1.2*s));
  c.save();
  c.globalCompositeOperation='lighter';
  c.globalAlpha=0.07+0.04*pulse;
  c.fillStyle='#fff3a0';
  fillPixelPoly(c,[
    [x-Math.round(5*s),headY-Math.round(2*s)],
    [x-Math.round(2*s),headY-Math.round(5*s)],
    [x+Math.round(3*s),headY-Math.round(5*s)],
    [x+Math.round(5*s),headY-Math.round(2*s)],
    [x+Math.round(3*s),headY+Math.round(1*s)],
    [x-Math.round(3*s),headY+Math.round(1*s)]
  ]);
  c.globalAlpha=0.70+0.16*pulse;
  c.fillStyle='#fff8c8';
  c.fillRect(x-Math.round(hw/2),headY-Math.round(3*s),hw,hh);
  c.fillStyle='#ffe070';
  c.fillRect(x-Math.round(hw/2)-Math.round(1*s),headY-Math.round(2*s),Math.max(1,Math.round(2*s)),hh);
  c.fillRect(x+Math.round(hw/2)-Math.round(1*s),headY-Math.round(2*s),Math.max(1,Math.round(2*s)),hh);
  c.globalAlpha=0.08;
  c.fillStyle='#ffe070';
  c.fillRect(x-Math.round(4*s),headY+Math.round(2*s),Math.round(8*s),Math.max(1,Math.round(1*s)));
  c.restore();
}
...
function drawRescueCages(c,rescues,cam,tk){
  ...
  for(const r of rescues){
    ...
    if(r.opened){
      ...
      c.fillStyle='#596070';
      c.fillRect(x-12,y-24,24,3);
      c.fillRect(x-12,y,6,3);c.fillRect(x+6,y,6,3);
      c.fillRect(x-12,y-24,3,27);c.fillRect(x+9,y-24,3,27);
      c.fillStyle='#7f8996';for(let xx=-6;xx<=6;xx+=6)c.fillRect(x+xx,y-22,2,22);
      c.fillStyle='#b7c1cc';c.fillRect(x-11,y-17,3,2);c.fillRect(x+8,y-17,3,2);c.fillRect(x-7,y+1,4,2);c.fillRect(x+3,y+1,4,2);
      const doorX=x-9,doorY=y+4;
      c.fillStyle='#4a5361';
      c.fillRect(doorX,doorY,18,2);c.fillRect(doorX+1,doorY+9,16,2);
      c.fillRect(doorX,doorY,2,11);c.fillRect(doorX+16,doorY,2,11);
      c.fillStyle='#9aa4b0';for(let dx=5;dx<=13;dx+=4)c.fillRect(doorX+dx,doorY+2,2,8);
      c.fillStyle='#d7e0ea';c.fillRect(doorX+2,doorY+1,6,1);
