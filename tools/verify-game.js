...
for (const token of ['UNDERWATER_SWIM_ACCEL','UNDERWATER_SWIM_RUN_ACCEL','UNDERWATER_SWIM_MAX','UNDERWATER_SWIM_RUN_MAX','UNDERWATER_SWIM_DRAG','UNDERWATER_OCTOPUS_WAKE_DELAY','UNDERWATER_OCTOPUS_GRAB_TICKS','UNDERWATER_OCTOPUS_DRAG_DEPTH','UNDERWATER_OCTOPUS_GONE_Y','UNDERWATER_OCTOPUS_GONE_TICKS','UNDERWATER_OCTOPUS_HOLY_REPEL_DELAY','UNDERWATER_OCTOPUS_HOLY_REPEL_TICKS']) {
  if (!underwaterRuntimeCode.includes(token)) {
...
if (!underwaterRuntimeCode.includes('UNDERWATER_OCTOPUS_DRAG_DEPTH=CH+96') || !underwaterRuntimeCode.includes("o.phase==='grab'") || !underwaterRuntimeCode.includes("phase==='holyRepel'") || !underwaterRuntimeCode.includes('cave.swimY>=UNDERWATER_OCTOPUS_GONE_Y') || !underwaterRuntimeCode.includes('o.goneT>=UNDERWATER_OCTOPUS_GONE_TICKS')) {
  throw new Error('Underwater octopus grab should pull mortal lemmels fully below the view and branch holy lemmels into repel');
}
if (!underwaterRuntimeCode.includes('startUnderwaterCaveHolyOctopusRepel') || !underwaterRuntimeCode.includes("o.phase='holyRepel'") || !underwaterRuntimeCode.includes("'FIAT LUX'") || !underwaterRuntimeCode.includes("'LUX VINCIT TENEBRAS'") || !underwaterRuntimeCode.includes('AU.sUnderwaterHolyRepel')) {
  throw new Error('Holy lemmels should repel the underwater octopus with a dedicated light sequence instead of dying');
}
...
for (const token of ['drawUnderwaterCaveView','drawUnderwaterMap','drawUnderwaterLemming','drawUnderwaterObjects','underwaterCaveLitRoom','drawUnderwaterCaveDarkness','drawUnderwaterHolyLight','drawUnderwaterHolyRepelLight','drawUnderwaterHolyRepelText','drawUnderwaterLampDiveDarkness','drawUnderwaterManualLampCutout','drawUnderwaterManualLampLight','drawUnderwaterOctopusThreat','drawUnderwaterOctopusWarning','createRadialGradient']) {
...
if (!underwaterRenderCode.includes('FIAT LUX') || !underwaterRenderCode.includes('LUX VINCIT TENEBRAS') || !underwaterRenderCode.includes("o.phase!=='holyRepel'") || !underwaterRenderCode.includes('repelP')) {
  throw new Error('Holy octopus repel should render Latin text and an explicit light-over-darkness animation');
}
...
if (!caveRenderCode.includes('drawWaterfallCaveEmberCampOpeningLight') || !caveRenderCode.includes('drawWaterfallCaveTorchGlow') || caveRenderCode.includes('[[CW,94],[CW,CH],[278,CH]') || caveRenderCode.includes('[[122,146],[44,92]') || caveRenderCode.includes('[[400,190],[450,174],[CW,184]') || caveRenderCode.includes('[[x-38,y+5],[x-17,y-31]') || caveRenderCode.includes('[[x,y-18],[x-70,y+28]')) {
  throw new Error('Ember passage should use localized torch glow and opening-only campfire light');
}
...
if (!caveRenderCode.includes('function drawWaterfallCaveChurchAltar(c)') || caveRenderCode.includes("#6a4724") || caveRenderCode.includes("#4d321b") || caveRenderCode.includes("#b9914c")) {
  throw new Error('Church altar foreground should reuse the same altar drawing instead of changing shape or color');
}
...
if (!audioCode.includes('sWaterfallCaveTeleportStone') || !audioCode.includes('sWaterfallCaveTeleportCharge') || !audioCode.includes('sWaterfallCaveRuneDiscover') || !audioCode.includes('sWaterfallCaveRunesComplete') || !audioCode.includes('sUnderwaterHolyRepel')) {
...
}
{
  ...
  const prevResumeMusic = G.underwaterCaveResumeMusic;
  ...
  const water = LEVELS[waterIdx].water.find(w => w && !w.lava);
  const holy = new Lemming(water.x + Math.min(12, Math.max(4, water.w/2)), water.y - 8);
  holy.holy = true;
  holy.state = 'MANUAL';
  ...
  }
  {
    ...
    const panicStartBeforeReturn = panicStarts;
    const underwaterStopBeforeReturn = underwaterStops;
    G.underwaterCave.swimX = 60;
    G.underwaterCave.swimY = 184;
    G.handleUnderwaterCaveKey('ArrowLeft');
    G.tick();
    G.handleUnderwaterCaveKeyUp('ArrowLeft');
    if (!G.underwaterCaveActive() || G.underwaterCave.scene !== 'entryPool' || !G.underwaterCave.octopus) {
      throw new Error('Returning to the entry pool as a manual diver with fins should bring the octopus back');
    }
    if (panicStarts !== panicStartBeforeReturn + 1 || underwaterStops <= underwaterStopBeforeReturn || AU.mus.kind !== 'underwaterPanic') {
      throw new Error('Returning to the entry pool with the octopus should restart panic music');
    }
    G.exitUnderwaterCave('silent');
    G.underwaterCaveExitCooldown = 0;
    {
...
}
{
  ...
  const prevHolyRepelSfx = AU.sUnderwaterHolyRepel;
  let failJingles = 0, holyRepels = 0;
  AU.jingle = win => { if (!win) failJingles++; };
  AU.sUnderwaterHolyRepel = () => { holyRepels++; };
  ...
  const fastEscape = new Lemming(184, 104);
  fastEscape.holy = true;
  G.lems = [fastEscape];
  G.manual = {used:true,active:true,lemId:fastEscape.id,lampOn:false,keys:{},jumpQueued:null,aimAngle:0};
  G.levelForceFail = false;
  ...
  if (!G.underwaterCaveActive() || G.underwaterCave.octopus.phase !== 'holyRepel' || holyRepels < 1 || !(G.underwaterCave.octopus.latinLines || []).includes('FIAT LUX')) {
    throw new Error('Octopus grab should trigger the holy repel sequence instead of killing a holy lemmel');
  }
  if (!drawUnderwaterCaveView(WCTX, 17)) {
    throw new Error('Holy octopus repel sequence did not render');
  }
  ...
  for (let i = 0; i < 430 && G.underwaterCaveActive(); i++) G.updateUnderwaterCave();
  if (G.underwaterCaveActive() || G.state !== 'PLAY' || G.levelForceFail || caught.dead || !G.manual.active) {
...
}
{
  ...
  const prevCrystalChime = AU.sWaterfallCaveCrystalChime;
  const prevTeleportChargeSound = AU.sWaterfallCaveTeleportCharge;
  ...
  const prevSetChurchHymnDistantLevel = AU.setWaterfallCaveChurchHymnDistantLevel;
  ...
  AU.sWaterfallCaveTeleportCharge = () => { crystalChargeSounds++; };
  ...
  AU.startWaterfallCaveChurchHymnDistant = (fade, level) => { churchHymnDistantStarted++; churchHymnFadeDurations.push(fade); churchHymnLevels.push({level, fade, start:true}); };
  AU.setWaterfallCaveChurchHymnDistantLevel = (level, fade) => { churchHymnLevels.push({level, fade}); };
  ...
  for (const id of ['main','deep','camp','emberPassage','crystalGallery','mirrorPool','glyphArchive','church','churchInterior']) {
  ...
  if (!deepItem.coverOpen || deepItem.coverSide !== 'front') {
    throw new Error('Deep cave game cover did not open with Space when prompted');
  }
  G.handleWaterfallCaveInput({x:20,y:20}, 'click');
  G.waterfallCave.lemX = deepItem.x - 72;
  G.waterfallCave.lemY = deepItem.y;
  ...
  if (!(songCrystal.obj.hintT > 0) || !Array.isArray(songCrystal.obj.hintLines) || !songCrystal.obj.hintLines.join(' ').includes('LADDAS')) {
    throw new Error('Song crystal should subtly hint that something can be charged before the teleport stone is found');
  }
  const chargeSoundsBefore = crystalChargeSounds;
  ...
  const throwStonePile = G.waterfallCaveSceneObjects(G.waterfallCave).find(hit => hit && hit.def && hit.def.id === 'mirrorThrowStones');
  if (!throwStonePile) throw new Error('Mirror pool is missing throwable stones');
  G.waterfallCave.lemX = throwStonePile.obj.x;
  G.waterfallCave.lemY = throwStonePile.obj.y;
  G.tick();
  if (throwStonePile.obj.activated || G.waterfallCave.mirrorStoneHeld) {
    throw new Error('Mirror pool stones should not be picked up just by walking over them');
  }
  G.handleWaterfallCaveKey(' ');
  if (!G.waterfallCave.mirrorStoneHeld || !throwStonePile.obj.activated || !(throwStonePile.obj.pickedT > 0)) {
    throw new Error('Mirror pool stone was not picked up with Space');
  }
  if (!drawWaterfallCaveView(WCTX, 44)) throw new Error('Mirror pool carried stone view did not render');
  ...
  if (!drawWaterfallCaveView(WCTX, 45)) throw new Error('Mirror pool thrown stone view did not render');
  ...
  if (!churchModel.def.blocker || !G.waterfallCaveSceneBlockerAt(G.waterfallCave, churchModel.obj.x - 42, churchModel.obj.y + 34)) {
    throw new Error('Church model should be a blocking cave object');
  }
  if (G.waterfallCaveSceneBlockerAt(G.waterfallCave, churchModel.obj.x + 12, churchModel.obj.y + 26)) {
    throw new Error('Church door should remain passable so the lemmel can enter');
  }
  ...
  G.waterfallCave.lemX = churchModel.obj.x + 12;
  G.waterfallCave.lemY = churchModel.obj.y + 26;
  G.handleWaterfallCaveKey('ArrowUp');
  G.tick();
  G.handleWaterfallCaveKeyUp('ArrowUp');
  if (!G.waterfallCaveActive() || G.waterfallCave.scene !== 'churchInterior') {
    throw new Error('Walking into the church door should enter the church interior');
  }
  if (G.waterfallCaveSceneDef('churchInterior').audio !== 'church-hymn' || churchHymnStarted < 1) {
    throw new Error('Church interior should start the MP3 hymn audio');
  }
  ...
  if (!(G.waterfallCave.blessingMessageT > 0) || !(G.waterfallCave.blessingMessageLines || []).join(' ').includes('OD')) {
    throw new Error('Priest blessing should show the immortal blessing text inside the church view');
  }
  ...
  if (G.holyTeleportStoneCharged || !Array.isArray(G.waterfallCave.teleportStoneMessageLines) || !G.waterfallCave.teleportStoneMessageLines.join(' ').includes('OLADDAD')) {
    throw new Error('Newly discovered teleport stone should start uncharged and explain that it must be charged');
  }
  ...
  const distantLevelsBeforeLeavingChurch = churchHymnLevels.length;
  ...
  if (churchHymnStopped !== hymnStoppedBeforeChurchExit || !G.waterfallCave.churchHymnDistant || !G.waterfallCave.churchHymnCarry) {
    throw new Error('The distant hymn should continue into the glyph archive but not restart as normal music');
  }
  ...
  const distanceLevels = churchHymnLevels.slice(distantLevelsBeforeLeavingChurch).map(v => Number(v && v.level)).filter(Number.isFinite);
  if (!distanceLevels.length || Math.min(...distanceLevels) >= Math.max(...churchHymnLevels.map(v => Number(v && v.level)).filter(Number.isFinite))) {
  ...
  if (churchHymnStopped <= hymnStoppedBeforeLeavingArchive || G.waterfallCave.churchHymnDistant || G.waterfallCave.churchHymnCarry) {
    throw new Error('Leaving the glyph archive for the mirror pool should stop the distant hymn');
  }
  G.waterfallCave.lemX = 240;
  G.waterfallCave.lemY = G.waterfallCaveSceneBounds(G.waterfallCave).minY;
  G.handleWaterfallCaveKey('ArrowUp');
  ...
  if (!G.portalStoneButtonAvailable()) {
    throw new Error('Charged teleport stone HUD button should be available for the holy hatch lemmel');
  }
  G.paused = true;
  ...
  if (G.holyTeleportStoneCharged || G.portalStoneButtonAvailable()) {
    throw new Error('Teleport stone charge should be consumed when the portals are opened');
  }
  ...
  AU.sWaterfallCaveCrystalChime = prevCrystalChime;
  AU.sWaterfallCaveTeleportCharge = prevTeleportChargeSound;
  ...
  AU.setWaterfallCaveChurchHymnDistantLevel = prevSetChurchHymnDistantLevel;
